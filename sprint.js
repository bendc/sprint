...
(function()
   {"use strict";
    ...
    var addPx = (function()
       {var noPx =["animation-iteration-count", "column-count", "flex-grow", "flex-shrink", "font-weight", "line-height", "opacity", "order", "orphans", "widows", "z-index" ];
   ...
   , beforeend: function(el)
    ...
    var duplicateEventListeners = function(el, clone)
       {// Element nodes only
       if (el.nodeType > 1) return;// Duplicate event listeners for the parent element...
        var listeners = getEvents(el);
        listeners && addEventListeners(listeners, clone);// ... and its descendants.
        var descendants = selectElements( "*", el);
        var descendantsLen = descendants.length;// cloneDescendants is defined later to avoid calling selectElements() if not needed
    ...
    var getSetDimension = function(obj, prop, value)
       ...
       if (value == null)
           {var el = obj.get(0);
            ...
            var capitalizedProp = prop[0].toUpperCase() + prop.substring(1);// dimension of HTML document
            if (el == document)
    ...
    var insertHTML = function(position, args)
       {var argsLen = args.length;
        var contents = args;// reverse argument list for afterbegin and afterend
    ...
    var sanitize = function(arr, flattenObjects, requireDomNodes)
       {/*
         ...
         * to DOM text nodes.
         */var arrLen = arr.length;
        var i = arrLen;// Check if arr needs to be sanitized first (significant perf boost for the most common case)
        while (i--)
           {// arr needs to be sanitized
           if (( ! arr[i] && arr[i] !== 0) || (flattenObjects && arr[i] instanceof Init) || (requireDomNodes && (typeof arr[i] == 
              "string" || typeof arr[i] == "number")))
               {var sanitized =[];
    ...
    var selectElements = function(selector, context)
       {context = context || document;// class, id, tag name or universal selector
        if ( /^[\#.]?[\w-]+$/.test(selector))
    ...
    var wrapMap =  {legend:  {intro: "<fieldset>", outro: "</fieldset>" }, area:  {intro: "<map>", outro: "</map>" }, param:  {intro: "<object>", outro: "</object>" }, thead:  {intro: "<table>", 
      outro: "</table>" }, tr:  {intro: "<table><tbody>", outro: "</tbody></table>" }, col:  {intro: "<table><tbody></tbody><colgroup>", outro: "</colgroup></table>" }, 
      td:  {intro: "<table><tbody><tr>", outro: "</tr></tbody></table>" }};// elements needing a construct already defined by other elements
    ["tbody", "tfoot", "colgroup", "caption"].forEach(function(tag)
       {wrapMap[tag] = wrapMap.thead;
       });
    wrapMap.th = wrapMap.td;// constructor
    ...
    var Init = function(selector, context)
       {if (typeof selector == "string")
           ...
           // select DOM elements
               ...
               {this.dom = context && context instanceof Init ? context.find(selector).get(): selectElements(selector, 
                  context);
               }
           }
   ...
   , css: function(property, value)
       {var valueType = typeof value;
        var isString = valueType == "string";// set
   ...
   , filter: function(selector)
       {var isFunc = typeof selector == "function";
        ...
        return this.map(function(i)
           {if (this.nodeType > 1 || ( ! isFunc && ! self.is(selector, this)) || (isFunc && ! selector.call(this, i, 
              this))) return;
   ...
   , find: function(selector)
       ...
       // .find(element)
        ...
        for (var i = 0;i < this.length;i++)
           {var el = this.get(i);
            if (el.nodeType > 1) continue;// check if each element in `this` contains the elements to find
            for (var j = 0;j < elementsToFindLen;j++)
               {var elementToFind = elementsToFind[j];
                ...
                if (elementsFoundLen < elementsToFindLen) continue;// everything has been found, return results
        ...
        if (typeof coordinates == "object")
           {return this.each(function()
               {if (this.nodeType > 1) return;
                ...
                $this.css("position") == "static" ? $this.css("position", "relative"): $this.css( {top: 0, left: 0 });
                var pos = $this.offset();
                $this.css( {top: coordinates.top - pos.top + "px", left: coordinates.left - pos.left + "px" });
               });
           }
               ...
               {if ( ! getEvents(this))
                ...
                eventsArr.forEach(function(event)
                   {if ( ! getEvents(this)[event])
                    ...
                    getEvents(this)[event].push(handler);// Ensure we add both the standard event (eg: "click") and the full event
   ...
   , position: function()
       {var pos =  {first: this.offset(), prt: this.parent().offset()};
        if ( ! pos.first) return;
        return  {top: pos.first.top - pos.prt.top, left: pos.first.left - pos.prt.left };
       }
